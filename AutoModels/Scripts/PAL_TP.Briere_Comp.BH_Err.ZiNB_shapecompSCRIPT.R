### This script builds and fits the model: PAL_TP.Briere_Comp.BH_Err.ZiNB_shapecomp
 ## Last built on:2024-01-12 15:29:31.270488
 ## Author Chris Terry
...
PAL_TP.Briere_Comp.BH_Err.ZiNB_shapecomp_fit <- brm(MODEL, data =  LaggedFemales_PAL,
prior = PRIORS, chains=4,cores =4, iter=4000)
 PAL_TP.Briere_Comp.BH_Err.ZiNB_shapecomp_fit <- add_criterion(PAL_TP.Briere_Comp.BH_Err.ZiNB_shapecomp_fit,criterion = c("loo", "waic"))

FIT <- PAL_TP.Briere_Comp.BH_Err.ZiNB_shapecomp_fit 
save(PAL_TP.Briere_Comp.BH_Err.ZiNB_shapecomp_fit , file ="../AutoModels/Fits/FEMS_PAL_TP.Briere_Comp.BH_Err.ZiNB_shapecomp")
 print(Sys.time())
