### This script builds and fits the D. pandora model: PAN_TP.Briere_Comp.BHTEMP_Err.Pois
 ## Last built on:2023-12-11 17:51:25.15099
 ## Author Chris Terry
...
B0+Tmin+Tmax+alphaii0+alphaij0+alphaiiT+alphaijT~1, nl = TRUE, family = poisson(link = "identity"))


PAN_TP.Briere_Comp.BHTEMP_Err.Pois_fit <- brm(MODEL, data =  LaggedFemales_PAN,
prior = PRIORS, chains=4,cores =4, iter=4000)
 PAN_TP.Briere_Comp.BHTEMP_Err.Pois_fit <- add_criterion(PAN_TP.Briere_Comp.BHTEMP_Err.Pois_fit,criterion = c("loo", "waic"))

FIT <- PAN_TP.Briere_Comp.BHTEMP_Err.Pois_fit 
save(PAN_TP.Briere_Comp.BHTEMP_Err.Pois_fit , file ="../AutoModels/Fits/FEMS_PAN_TP.Briere_Comp.BHTEMP_Err.Pois")
 print(Sys.time())
