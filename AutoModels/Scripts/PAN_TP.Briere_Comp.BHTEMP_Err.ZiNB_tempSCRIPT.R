### This script builds and fits the D. pandora model: PAN_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp
 ## Last built on:2023-12-11 17:51:25.173804
 ## Author Chris Terry
...
PAN_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp_fit <- brm(MODEL, data =  LaggedFemales_PAN,
prior = PRIORS, chains=4,cores =4, iter=4000)
 PAN_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp_fit <- add_criterion(PAN_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp_fit,criterion = c("loo", "waic"))

FIT <- PAN_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp_fit 
save(PAN_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp_fit , file ="../AutoModels/Fits/FEMS_PAN_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp")
 print(Sys.time())
