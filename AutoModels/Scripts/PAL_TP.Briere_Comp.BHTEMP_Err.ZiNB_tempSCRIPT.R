### This script builds and fits the model: PAL_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp
 ## Last built on:2024-01-12 15:29:31.356667
 ## Author Chris Terry
...
PAL_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp_fit <- brm(MODEL, data =  LaggedFemales_PAL,
prior = PRIORS, chains=4,cores =4, iter=4000)
 PAL_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp_fit <- add_criterion(PAL_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp_fit,criterion = c("loo", "waic"))

FIT <- PAL_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp_fit 
save(PAL_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp_fit , file ="../AutoModels/Fits/FEMS_PAL_TP.Briere_Comp.BHTEMP_Err.ZiNB_temp")
 print(Sys.time())
