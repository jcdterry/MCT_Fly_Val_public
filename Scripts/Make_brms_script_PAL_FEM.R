Make_brms_script_PAL_FEM <- function(i,Option_df , cores = 4, iter = 2000){
  
  TP = Option_df$TP[i]
  Comp = Option_df$Comp[i]
  Err = Option_df$Err[i]
  
  Name = paste(paste0('PAL_TP.', TP)   ,
               paste0('Comp.', Comp)   ,
               paste0('Err.', Err)   ,
               sep = '_')
  ...
  if( TP == 'Atkin'){ 
    TP_Priors <- c('set_prior("normal(80, 15)", nlpar = "B0", lb = 10, ub = 200)',
                   'set_prior("normal(3, 50)", nlpar = "a", lb = 1, ub=100)',
                   'set_prior("normal(0.5, 5)", nlpar = "b", lb = 0.0001, ub=10)')
    TP_function <- '(B0 * (a - b * PrevTemp)^(PrevTemp/10))'
  ...
  if( TP ==   'GG'){  ## GaussianGompertz ## nb labels vary a bit between code and pdf
    TP_Priors <- c('set_prior("normal(80, 15)", nlpar = "B0", lb = 10, ub = 200)',
                   'set_prior("normal(3, 50)", nlpar = "b",   lb = 0, ub=50)',
                   'set_prior("normal(2, 50)", nlpar = "c",   lb = 0, ub=50)',
                   'set_prior("normal(2, 50)", nlpar = "Tpk", lb = 20, ub=35)')
    TP_function <- ' B0 * exp(-exp(b*(PrevTemp-Tpk) -6 ) - c*(PrevTemp-Tpk)^2)'
    TP_NL =  paste0( c( "B0",   "b",  "c",   "Tpk"), collapse = '+')
  } 
  ...
  if( Comp  == 'BH'){
    ...
    Comp_function <- '(1/(1+alphaii*Prev_PAL_FEMALE_comp + alphaij*Prev_PAN_FEMALE))'
  ...
  if( Comp  == 'BHBeta'){
    Comp_Priors <-  c('set_prior("normal(1, 5)", nlpar = "beta", lb =0.1, ub=5)',
                      ...
                      'set_prior("normal(0.00001, 0.5)", nlpar = "alphaij", lb = 0.00001, ub=20)')
    Comp_function <- '(1/((1+alphaii*Prev_PAL_FEMALE_comp + alphaij*Prev_PAN_FEMALE)^beta))'
  ...
  if( Comp  == 'BHTEMP'){
    ...
    Comp_function <- '(1/((1+(alphaii0+alphaiiT*Temp20)*Prev_PAL_FEMALE_comp + (alphaij0+alphaijT*Temp20)*Prev_PAN_FEMALE)))'
  ...
  if( Comp  == 'BHTEMPBeta'){
    ...
    Comp_function <- '(1/((1+(alphaii0+alphaiiT*Temp20)*Prev_PAL_FEMALE_comp + (alphaij0+alphaijT*Temp20)*Prev_PAN_FEMALE)^beta))'
    Comp_NL =  paste0( c("alphaii0" ,"alphaij0",  "alphaiiT" ,"alphaijT" , 'beta'), collapse = '+')
  }
  if( Comp  == 'BHTEMP1'){
    Comp_Priors <-  c('set_prior("normal(0.00001, 0.5)", nlpar = "alphaii0", lb = 0.00001, ub=20)',
                      'set_prior("normal(0, 0.05)", nlpar = "alphaiiT", lb = -2, ub=2)',
                      'set_prior("normal(0.00001, 1)", nlpar = "alphaij", lb = 0.00001, ub=20)')
    
    Comp_function <- '(1/((1+(alphaii0+alphaiiT*Temp20)*Prev_PAL_FEMALE_comp + alphaij*Prev_PAN_FEMALE)))'
    Comp_NL =  paste0( c("alphaii0" ,"alphaij",  "alphaiiT"), collapse = '+')
  }
  if( Comp  == 'BHTEMP1Beta'){
    Comp_Priors <-  c('set_prior("normal(1, 5)", nlpar = "beta", lb = 0.1, ub=5)',
                      'set_prior("normal(0.00001, 0.5)", nlpar = "alphaii0", lb = 0.00001, ub=20)',
    ...
    Comp_function <- '(1/((1+(alphaii0+alphaiiT*Temp20)*Prev_PAL_FEMALE_comp + alphaij*Prev_PAN_FEMALE)^beta))'
    Comp_NL =  paste0( c("alphaii0" ,"alphaij",  "alphaiiT", 'beta'), collapse = '+')
  }
  if( Comp  == 'BHTEMP1Log'){
    Comp_Priors <-  c( 'set_prior("normal(0.00001, 0.5)", nlpar = "alphaii0", lb = 0.00001, ub=20)',
                       'set_prior("normal(0, 0.05)", nlpar = "alphaiiT", lb = -2, ub=2)',
    ...
    Comp_function <- '(1/((1+(alphaii0+alphaiiT*Temp20)*log(Prev_PAL_FEMALE_comp+1) + alphaij*log(Prev_PAN_FEMALE+1))))'
    Comp_NL =  paste0( c("alphaii0" ,"alphaij",  "alphaiiT"), collapse = '+')
  }
  if( Comp  == 'BHTEMP1LogBeta'){
    ...
    Comp_function <- '(1/((1+(alphaii0+alphaiiT*Temp20)*log(Prev_PAL_FEMALE_comp+1) + alphaij*log(Prev_PAN_FEMALE+1))^beta))'
    Comp_NL =  paste0( c("alphaii0" ,"alphaij",  "alphaiiT", 'beta'), collapse = '+')
  }
  ...
  if(Err == 'ZiNB_comp'){
    Err_Priors = NULL
    Err_family = 'zero_inflated_negbinomial(link = "identity")' 
    Err_function = 'zi~Prev_PAL_FEMALE,'
  }
  ...
  if(Err == 'ZiNB_shapecomp'){
    Err_Priors = NULL
    Err_family = 'zero_inflated_negbinomial(link = "identity")' 
    Err_function = 'shape~Prev_PAL_FEMALE,'
  }
  
  Intro = paste0('### This script builds and fits the model: ', Name,
  ...
  ModelFunction <- paste0('\n\nMODEL<-bf(PAL_FEMALE~Prev_PAL_FEMALE*',TP_function, '*', Comp_function, ',\n',
                          Err_function, '\n',
                          NLs, '~1, nl = TRUE, family = ', Err_family, ')\n\n')
  
  
  ModelFit <- paste0( Name, '_fit <- brm(MODEL, data =  LaggedFemales_PAL,\n',
